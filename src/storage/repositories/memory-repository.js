...
class MemoryRepository {
  ...
  async add(input, options = {}) {
    const params = normalizeMemoryInput(input);
    ...
    return withTenantTransaction(
      ...
      async (client) => {
        ...
        return result.rows[0]
          ? { id: result.rows[0].id, forgottenAt: result.rows[0].forgotten_at }
          : null;
      },
      options
    );
...
function normalizeMemoryInput(input = {}) {
  ...
  return {
    ...
    content,
    normalizedKey: normalizeNullableText(input.normalizedKey),
