...
async function main() {
  ...
  const storage = createStorage({
    databaseApplicationName: "mji-admin-hub",
  ...
  server.listen(port, HOST, () => {
    ...
    console.log("\nM叽管理员总后台已启动");
    console.log(`地址：${url}`);
    console.log("顶部可切换：余额管理 / 人设管理 / 扫码接入");
...
async function handleRequest({ request, response, storage, tenant, config, adminToken, templates, sessions }) {
  ...
  if (method === "GET" && pathname === "/") {
    sendHtml(response, renderTemplate(templates.hub, tenant, adminToken), { frame: false });
    return;
  }
  if (method === "GET" && pathname === "/wallet") {
    sendHtml(response, renderTemplate(templates.wallet, tenant, adminToken), { frame: true });
    return;
  }
  if (method === "GET" && pathname === "/persona") {
    sendHtml(response, renderTemplate(templates.persona, tenant, adminToken), { frame: true });
    return;
  }
  if (method === "GET" && pathname === "/onboard") {
    sendHtml(response, renderTemplate(templates.onboard, tenant, adminToken), { frame: true });
    return;
  }
