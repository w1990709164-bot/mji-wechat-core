...
async function resolveExactTarget({ storage, userId, accountId }) {
  ...
  return storage.withTenant(tenantId, async (client) => {
      ...
      `SELECT
       ...
       LEFT JOIN LATERAL (
         ...
         WHERE tenant_id = u.tenant_id
           AND user_id = u.id
           AND LOWER(channel_account_id) = LOWER($3)
    ...
    if (!row.provider_user_id || !row.channel_account_id) {
      throw new Error(`指定用户没有绑定到机器人账号 ${accountId}`);
    }
    if (normalizeAccountId(row.channel_account_id) !== normalizeAccountId(accountId)) {
